...
| `EceServiceClosure` | `ClosureStartDate`, `ClosureEndDate`, `ClosureReasonCode` | ~~**Nothing**~~ **`service_closures` (`0088`)**, shaped as the event asks. `booking_status`'s `closed` value stays what it was — a per-child-day statement, not a service one. Two gaps remain: the reason is a `LookupCode` with **no published list**, so it ships unresolvable; and a closure with no stated end has no `ClosureEndDate` to send |
...
| Event | Then | Now |
|---|---|---|
| `ChildBookingSchedule` | Nothing mapped | **`0085`** — effective-dated weekday blocks with times, keyed on the child as the XSD is. **Screen shipped 2026-09-04** and three funding rules read it; serialiser outstanding |
...
| `EceServiceClosure` | Nothing | **`0088`** — `service_closures`. Built because it is **load-bearing in four places at once**, not for the event: §6-6 suspends the Three Week Rule while a service is closed for two weeks or more, RS7's `AdvanceMonthCounts` needs forward operating days, `averageOverOpenDays` currently infers a closed day from nobody attending, and the event maps here |
